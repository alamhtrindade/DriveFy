...
public class Agendar extends javax.swing.JFrame {
    ...
    private iAgendamento agendamentos;
    Banco b = new Banco();
     ...
     */
    public Agendar(iAgendamento agendamento) {
        initComponents();
        carregarServicos();
...
//valida e grava tudo se gravar for verdadeiro
            Agenda agenda;
            agenda = new Agenda(cliente.getCpf(), placaSelecionada, profissional.getCpf(), servico.getNome_servico(), data, hora, status,inputObservacoes.getText() , servico.getPreco());

            if (b.cadastrarAgenda(agenda)) {
                JOptionPane.showMessageDialog(rootPane, "Agendamento para " + cliente.getNome() + " e placa " + placaSelecionada + " cadastrado com sucesso!");
                agendamentos.atualizarTabela();
                this.dispose();
            } else {
                JOptionPane.showMessageDialog(rootPane, "Agendamento para " + cliente.getNome() + " não cadastrado, validar se já não existe ou se os campos estão corretos!");
            }

            this.dispose();
        } else {
            JOptionPane.showMessageDialog(rootPane, "Não é possivel gerar agendamento com os dados atuais, gentileza verificar os dados imputados!");
        }
